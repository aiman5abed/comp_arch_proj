...
static bool is_reg_in_flight(Core* core, int reg) {
    ...
    int id_ex_dest = get_dest_reg(&core->ID_EX);
    if (id_ex_dest == reg) {
        return true;
    }
    
    // Check EX_MEM (will write in future)
    int ex_mem_dest = get_dest_reg(&core->EX_MEM);
    if (ex_mem_dest == reg) {
        return true;
    }
    
    // Check MEM_WB (will write this cycle, but not visible until next cycle)
    int mem_wb_dest = get_dest_reg(&core->MEM_WB);
    if (mem_wb_dest == reg) {
        return true;
    }
...
static bool do_decode(Core* core, Simulator* sim, PipelineLatch* next_ID_EX, 
    ...
    *next_ID_EX = core->IF_ID;
    ...
    // Per spec: branch target = R[rd][9:0] (low 10 bits of register rd VALUE)
    int32_t rs = next_ID_EX->rs_val;
    int32_t rt = next_ID_EX->rt_val;
    uint32_t target = (uint32_t)next_ID_EX->rd_val & PC_MASK;  // Low 10 bits of R[rd]
